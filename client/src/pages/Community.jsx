      ...
      {images.length > 0 ? (
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {images.map((item, index) => (
            ...
            <a href={item.imageUrl} key={index} target='_blank' className='relative group block rounded-lg overflow-hidden 
            border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md
             transition-shadow duration-300'>

              <img className='w-full h-40 md:h-50 2xl:h-62 object-cover group-hover:scale-105 transition-transform
