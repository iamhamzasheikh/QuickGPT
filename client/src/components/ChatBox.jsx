...
const ChatBox = () => {
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      {mode === 'image' && (
        <label className='inline-flex items-center gap-2 pb-2.5 text-sm mx-auto'>
          <p className='text-xs'>Publish Generated Image to Community</p>
          <input className='cursor-pointer' type="checkbox" checked={isPublished}
      ...
      <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary
       dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>
        <select onChange={(e) => setMode(e.target.value)} value={mode}
          className='text-sm pl-3 pr-2'>
          <option className='dark:bg-purple-900' value="text">Text</option>
          <option className='dark:bg-purple-900' value="image">Image</option>
        </select>

        <input className='flex-1 w-full text-sm outline-none'
          required type="text"
          value={prompt}
