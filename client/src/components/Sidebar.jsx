  ...
  return (
    ...
    <div className={`flex flex-col h-screen min-w-72 p-5 
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input type="text" placeholder='Search conversation' onChange={(e) => setSearch(e.target.value)} value={search}
          className='text-xs placeholder:text-gray-400 outline-none' />
      </div>

      {/* Recant-Chats */}

      {chats.length > 0 && <p className='mt-4 text-sm'>Recant Chats</p>}

      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.
            ...
            chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
              ...
              dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
                ...
                <div>
                  ...
                  <p className='text-xs text-green-500 dark:text-[#B1A6C0]'>{moment(chat.updatedAt).fromNow()}</p>
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-3 mt-4 border border-gray-300
       dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <img src={assets.diamond_icon} alt="" className='w-4.5 to-dark:invert' />
      ...
      <div className='flex items-center gap-2 justify-between p-3 mt-4 border border-gray-300
      ...
      <div className='flex items-center gap-4 p-3 mt-4 border border-gray-300
